`timescale 1ns/1ps

// eFPGA Wrapper Top Module
module eFPGA #(
  parameter int V = 3,
  parameter int H = 2,
  parameter int W = 12
) (
  //Programming Interface
  input wire              prog_nres,
  input wire              prog_clk,
  input wire              prog_start,
  //Register Inputs
  input wire              reg_nres,
  input wire              reg_clk,
  //Module In/Out
  input wire  [62*V+31:0] N_i,
  output wire [62*V+31:0] S_o,
  input wire  [34*V+15:0] S_i,
  output wire [34*V+15:0] N_o,
  input wire  [40*H+31:0] W_i,
  output wire [56*H+31:0] E_o,
  input wire  [16*H+15:0] E_i,
  output wire [16*H+15:0] W_o
);
  
  // Registers
  
  // Wires
  logic [31:0] dout;
  logic [W-1:0] addr;
  
  // Assigns

  // Instances

  rams_init_file #(.addrW(W)) bram (.clk(prog_clk), .we('0), .addr(addr), .din('0), .dout(dout));

  eFPGA_prog #(.V(V), .H(H), .W(W)) eFPGA_unit (.prog_nres(prog_nres), .prog_clk(prog_clk), .prog_start(prog_start),
    .addr(addr), .dout(dout), 
    .reg_nres(reg_nres), .reg_clk(reg_clk),
    .N_i(N_i), .S_o(S_o), .S_i(S_i), .N_o(N_o), .W_i(W_i), .E_o(E_o), .E_i(E_i), .W_o(W_o));
  
endmodule